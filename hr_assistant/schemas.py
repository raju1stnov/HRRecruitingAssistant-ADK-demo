# HRRecruitingAssistant-ADK-demo/hr_assistant/schemas.py

from pydantic import BaseModel, Field
from typing import List, Optional, Dict, Any # Ensure Dict and Any are imported if used in JSONRPCRequest
...
class SearchOutput(BaseModel):
    # Changed to optional error as per tool implementation returning None on success
...
class SaveCandidateOutput(BaseModel):
    # Changed to optional error/name as per tool implementation
    status: str = Field(..., description="'saved' on success, 'error' on failure")
    name: Optional[str] = Field(None, description="Name associated with the save attempt")
    error: Optional[str] = Field(None, description="Error message on failure")

# --- Schemas below are likely NOT needed for 'adk web' but don't hurt ---
...
# --- Schema for JSON-RPC A2A Calls (Needed by main.py /a2a endpoint if used) ---
